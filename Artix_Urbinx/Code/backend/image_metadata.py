...
def load_jpg_metadata(image_path):
        ...
        if exif_data:
            for tag_id, value in exif_data.items():
                ...
                if tag_name in ["ImageDescription", "UserComment"]:  
                    try:
                        # Attempt to parse the value as a dictionary if it contains structured metadata
                        metadata = ast.literal_eval(value) if isinstance(value, str) else value
                    except (SyntaxError, ValueError):
                        metadata[tag_name] = value  # Store raw value if parsing fails
