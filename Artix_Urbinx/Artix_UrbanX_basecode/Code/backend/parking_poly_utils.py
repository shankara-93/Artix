...
def json_geojson(json_dict):
    """
    Convert a JSON dictionary to a GeoDataFrame.
    """
    geojson_dict = json.loads(json_dict)
    return gpd.GeoDataFrame.from_features(geojson_dict["features"])

# def intersecting_geojson(gdf_centroids_dict, gdf_parking_dict):
#     """
...
#     grouped_df['occupancy'] = intersecting_points.groupby('osmid')['geometry'].count().values
#     grouped_df['area_m2'] = grouped_df['area_m2'].round(2)
#     parking_gdf = grouped_df.to_crs(crs) 
#     return parking_gdf
